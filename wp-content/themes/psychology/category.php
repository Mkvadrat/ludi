      ...
      <div class="row">
          ...
          <div class="col-md-8">
              <div class="articles-list">
                  <h1><?php echo $cat_data['title_for_categories_articles_page']; ?></h1>
...
<?php
							...
							if(!empty($articles)){
							foreach( $articles as $articles_list ){

								$image_url = wp_get_attachment_image_src( get_post_thumbnail_id($articles_list->ID), 'full');

								$short_descr = get_post_meta( $articles_list->ID, 'short_descr_post_page', $single = true );
?>
                  <div class="article-block">
										<?php if(!empty($image_url)){ ?>
											<img src="<?php echo $image_url[0]; ?>" alt="<?php echo get_post_meta( get_post_thumbnail_id($articles_list->ID), '_wp_attachment_image_alt', true ); ?>">
										<?php }else{ ?>
											<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/photo-12.jpg" alt="">
										<?php } ?>

                    <h2><?php echo wp_trim_words( $articles_list->post_title, 4,''); ?></h2>
											<?php if($short_descr){ ?>
                      	<p><?php echo wp_trim_words( $short_descr, 9, '<a href="'. get_permalink($articles_list->ID) .'"> ...Подробнее</a>' ); ?></p>
											<?php }else{ ?>
												<p><?php echo wp_trim_words( $articles_list->post_content, 9, '<a href="'. get_permalink($articles_list->ID) .'"> ...Подробнее</a>' ); ?></p>
											<?php } ?>
                  </div>
<?php
							}
...
?>
							...
							<?php foreach ($pagination as $pag){ ?>
									<li class="active"><?php echo $pag; ?></li>
							<?php } ?>
								</ul>
							<?php } ?>
